...
deque< pair<double, pose_ekf::Altimeter> >altimeter_q;
...
bool loadModels() 
{
  ...
  if (!geoid) {
    try {
      geoid = std::make_shared<GeographicLib::Geoid>("egm84-15", "/home/an/helloworld/src/pose_ekf/src/geoids");
    }
...
bool processSensorData()
{
  ...
  if(min_id == 0)//imu
  {
        ...
        imu_cnt++;
        if(imu_cnt % 10 == 0) ekf.correct_gravity(acc, t);
        imu_q.pop_front();
    ...
    } else if(min_id == 1)//magnetic 
    {
      ...
      mag(1) = msg.vector.y;
      mag(2) = msg.vector.z;
      ekf.correct_magnetic_field(mag, t);
      mag_q.pop_front();
    ...
    }else if(min_id == 3) //sonar height
    {
      ...
      ekf.correct_sonar_height(sonar_height, t);
    ...
    }else if(min_id == 5) //fix_velocity
    {
      ...
      ekf.correct_fix_velocity(fix_velocity, t);
...
void altimeterCallback(const pose_ekf::AltimeterConstPtr& msg)
{
  double t = msg->header.stamp.toSec();
